...
func (c *AcceptCommand) commandAcceptChangesetByReviewer(ev *github.IssueCommentEvent) (bool, error) {
	...
	if !c.repo.Reviewers().Has(sender) {
		log.Printf("%v is not an reviewer registred to this bot.\n", sender)
		return false, nil
	}
	...
	if err != nil {
		log.Println("could not change labels by the issue")
		return false, err
	}

	{
		...
		if err != nil {
			log.Println("could not create the comment to declare to merge this.")
			return false, err
		}
	}
	...
	if err != nil {
		log.Println("could not merge pull request")
...
func (c *AcceptCommand) commandAcceptChangesetByOtherReviewer(ev *github.IssueCommentEvent, command string) (bool, error) {
	...
	if len(tmp) < 2 {
		log.Println("No specify who is the actual reviewer.")
		return false, nil
	}

	actual := tmp[1]
	if !c.repo.Reviewers().Has(actual) {
		log.Println("could not find the actual reviewer in reviewer list")
		return false, nil
	}
